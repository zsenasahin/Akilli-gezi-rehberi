...
/**
 * Itinerary detail screen – manage an ongoing/completed itinerary.
 * Features:
 * - View day-by-day plan
 * - Mark places as visited (checkbox)
 * - Remove a place
...
const ItineraryDetailScreen = ({ route, navigation }) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        // Fetch all places for the city (for alternative suggestions)
        if (data?.city_id) {
    ...
    const handleRemoveItem = (item) => {
        Alert.alert(
            'Yeri Kaldır',
            `"${item.places?.name}" planından çıkarılsın mı?`,
            [
                { text: 'İptal', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        ...
                        if (removeError) {
                            Alert.alert('Hata', 'Yer kaldırılırken hata oluştu.');
                        } else {
                            setItinerary((prev) => ({
                                ...prev,
                                itinerary_items: prev.itinerary_items.filter(
                                    (i) => i.id !== item.id
                                ),
                            }));
                        }
    ...
    const handleSuggestAlternative = async (item) => {
        ...
        if (!suggestion) {
            Alert.alert('Bilgi', 'Alternatif yer bulunamadı.');
            return;
        }
    ...
    const handleCompleteItinerary = () => {
        Alert.alert(
            'Planı Tamamla',
            'Bu gezi planını tamamlandı olarak işaretlemek istiyor musunuz?',
            [
                { text: 'İptal', style: 'cancel' },
                {
                    text: 'Tamamla',
                    onPress: async () => {
                        const { error: statusError } = await updateItineraryStatus(
                            itinerary.id,
                            'completed'
                        );
                        if (statusError) {
                            ...
                            setItinerary((prev) => ({ ...prev, status: 'completed' }));
                            Alert.alert('Tebrikler! 🎉', 'Gezi planınız tamamlandı!');
                        }
                    },
                },
            ]
    ...
    if (!itinerary) {
        return <ErrorMessage message="Plan bulunamadı." />;
    }
                ...
                <View style={styles.headerStats}>
                    ...
                    <View style={styles.headerStat}>
                        <Text style={styles.headerStatValue}>
                            {itinerary.itinerary_items?.length || 0}
                        </Text>
                ...
                <View key={group.day} style={styles.dayCard}>
                    ...
                    {group.items.map((item) => (
                        <View
                            ...
                            style={[
                                styles.itemRow,
                                item.is_completed && styles.itemRowCompleted,
                            ]}
                        >
                            {/* Checkbox */}
                            <TouchableOpacity
                            ...
                            <View style={styles.itemInfo}>
                                ...
                                <Text style={styles.itemMeta}>
                                    ...
                                    {item.places?.entry_fee > 0
                                        ? `₺${item.places.entry_fee}`
                                        : 'Ücretsiz'}
                                </Text>
                            </View>
